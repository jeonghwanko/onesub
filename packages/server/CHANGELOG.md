...
- 25f4257: Point each package's `homepage` at https://onesub.pryzm.gg
...
- 5fd54c9: Name the product and app when a subscription is sent to the product endpoint.
...
- d66c230: Stop rejecting a legitimate restore of an already-consumed Google consumable.
...
- cab70d4: **Breaking in production:** `POST /onesub/webhook/google` now answers 401 when no
  ...
  The exposure this closes was entitlement _revocation_, not forged entitlement: the
...
- aa15af7: Finish the structured-logging migration: routes, the app registry, webhook
...
- 62f7812: Move Apple and Google provider log values out of the message and into named
  `key=value` fields.
  ...
  Log _text_ changes for these two providers; the `[onesub/apple]` and
...
- 3cff4eb: Render each log call into one escaped string, so a caller cannot forge a log line on
  any sink.
...
- 8db4354: Escape backslashes before line terminators when logging.
...
- deacf96: Rewrite the log line-break escaping so a taint analyser can see it.
...
- 8ebfdf5: Stop a caller from forging log lines.
...
- e95c4b9: Push metrics aggregation into SQL instead of reducing every row in the process.
...
- b12e8cc: Mount `POST /onesub/webhook/google` only when the config serves Google Play.
...
- f5fd788: Warn at startup when the Google RTDN route is left open.
  ...
  `orderId`. So the exposure is entitlement _revocation_ by a caller who knows or
...
- 7c54b1b: Stop 500 responses from describing the server, and unify input validation.
...
- e0635ee: Look up purchases by product instead of reading a user's whole history.
...
- 405f6aa: Add sandbox-only entitlement overrides for manual testing.
  ...
  covers sandbox _tester_ accounts. A developer who subscribes once to check the
...
- 9856a22: Validate receipts for N apps from one onesub instance.
...
- Updated dependencies [9856a22]
  - @onesub/shared@0.11.0
...
- Updated dependencies [91cdea6]
  - @onesub/shared@0.10.0

## 0.15.0

### Minor Changes

- 141150a: Google subscription lifecycle + validation hardening (review sweep).
  ...
  - **Apple webhook enforces `bundleId`** (400 `BUNDLE_ID_MISMATCH`, mirroring the Google packageName check) and **`CONSUMPTION_REQUEST` no longer revokes**: it is a refund _review_ request; only `REFUND`/`REVOKE` remove entitlement.
...
- 83ae9f3: Bind product purchase validation to the account identity baked into the receipt.
...
- a7715bb: Fix: already-owned non-consumable now returns the recorded transactionId
...
- 4cf6c6c: v1.0 hardening — cluster-safe runtime, webhook idempotency + queue, dual ESM/CJS build.
  ...
  **@onesub/server**
  ...
  - Add `WebhookEventStore` for inbound idempotency — `InMemoryWebhookEventStore` for single-instance, `CacheWebhookEventStore` for generic cache backends, and `RedisWebhookEventStore` for Redis (uses atomic `SET NX` — no get→set race). Apple `notificationUUID` and Google Pub/Sub `messageId` are now deduped before any state change when the store is configured.
  ...
  - `RedisSubscriptionStore.listAll()` now returns newest-first (sorted set vs random `SMEMBERS`). `listFiltered()` without secondary filters uses a single `ZREVRANGE + ZCARD` — O(log n + limit) instead of full-set pull.
...
- b7afee1: Two related additions backing the dashboard's customer detail page and lifetime-product host insights.
